...
class JmApiClient:
    ...
    def ranking(self, rank_type: str = "day", page: int = 1) -> List[AlbumMeta]:
        if rank_type == "week":
            params = {"page": str(max(1, page)), "order": "mv_w", "c": "0", "o": ""}
        elif rank_type == "month":
            params = {"page": str(max(1, page)), "order": "", "c": "0", "o": "mv_m"}
        else:
            params = {"page": str(max(1, page)), "order": "mv_t", "c": "0", "o": ""}
        data = self._api_get("/categories/filter", params)
        return self._parse_album_items(data.get("content"), ranked=True)
