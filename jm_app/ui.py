...
class AlbumCard(QFrame):
    ...
    def enterEvent(self, event):
        ...
        self._animate_hover(self._base_pos + QPoint(0, -6), 92)
        super().enterEvent(event)

    def leaveEvent(self, event):
        self._animate_hover(self._base_pos, 0)
        super().leaveEvent(event)

    def _animate_hover(self, target_pos: QPoint, alpha: int):
        shadow = self._ensure_shadow()
        self._hover_pos_animation = QPropertyAnimation(self, b"pos", self)
        self._hover_pos_animation.setDuration(180)
        self._hover_pos_animation.setStartValue(self.pos())
        self._hover_pos_animation.setEndValue(target_pos)
        self._hover_pos_animation.setEasingCurve(QEasingCurve.OutCubic)
        self._hover_pos_animation.start()

        color = shadow.color()
        self._hover_shadow_animation = QPropertyAnimation(shadow, b"color", self)
        self._hover_shadow_animation.setDuration(180)
        self._hover_shadow_animation.setStartValue(color)
        self._hover_shadow_animation.setEndValue(Qt.transparent if alpha <= 0 else self._shadow_color(alpha))
        self._hover_shadow_animation.setEasingCurve(QEasingCurve.OutCubic)
    ...
    def _ensure_shadow(self) -> QGraphicsDropShadowEffect:
        try:
            if self.shadow is not None:
                self.shadow.color()
                if self.graphicsEffect() is self.shadow:
                    return self.shadow
        except RuntimeError:
            pass
...
class MainWindow(QMainWindow):
    ...
    def connect_settings_autosave(self):
        ...
        for checkbox in [self.make_pdf, self.keep_images, self.stop_on_block, getattr(self, "detail_check", None)]:
            if checkbox:
    ...
    def enter_app(self, username: str = "", password: str = "", cookie: str = ""):
        ...
        self._resize_to_available(1220, 820)
        self._center_window()
    ...
    def _build_app_page(self) -> QWidget:
        ...
        content.setMinimumSize(900, 610)
    ...
    def _build_search_panel(self) -> QFrame:
        ...
        self.id_edit.setPlaceholderText("下载 ID：422866, p456")
        option_row.addWidget(self.id_edit, 1)
        start_btn = QPushButton("开始下载")
        start_btn.setObjectName("primaryButton")
        start_btn.clicked.connect(self.quick_start_download)
        option_row.addWidget(start_btn)
    ...
    def _build_result_panel(self) -> QFrame:
        ...
        self.result_grid.setContentsMargins(4, 4, 4, 4)
        self.result_grid.setHorizontalSpacing(4)
        self.result_grid.setVerticalSpacing(6)
        ...
        footer = QHBoxLayout()
        footer.setContentsMargins(10, 4, 10, 8)
    ...
    def _build_detail_panel(self) -> QFrame:
        ...
        self.cover_label.setStyleSheet(
            "background:#eef3f8;border:1px solid #dde6f0;border-radius:6px;color:#8491a5;"
        )
        self.detail_card = QLabel("未选择 ID")
        self.detail_card.setAlignment(Qt.AlignLeft | Qt.AlignTop)
        self.detail_card.setWordWrap(True)
        self.detail_card.setMinimumHeight(70)
        self.detail_card.setStyleSheet(
            "background:#f8fafc;border:1px solid #e0e7f2;border-radius:6px;color:#334155;padding:7px;"
        )
        self.tags_card = QLabel("标签：-")
        self.tags_card.setAlignment(Qt.AlignLeft | Qt.AlignTop)
        self.tags_card.setWordWrap(True)
        self.tags_card.setMinimumHeight(44)
        self.tags_card.setStyleSheet(
            "background:#f8fafc;border:1px solid #e0e7f2;border-radius:6px;color:#475569;padding:7px;"
        )
        ...
        layout.addWidget(load_chapter_btn)
        layout.addWidget(read_chapter_btn)
        chapter_btns = QHBoxLayout()
        chapter_btns.addWidget(all_chapter_btn)
        chapter_btns.addWidget(none_chapter_btn)
        ...
        self.chapter_table.setMaximumHeight(230)
        layout.addWidget(self.chapter_table)
        layout.addWidget(add_current_btn)
        return panel
    ...
    def _build_download_panel(self) -> QFrame:
        ...
        self.cancel_btn = QPushButton("暂停/取消")
        ...
        self.queue_table.setHorizontalHeaderLabels(["ID", "漫画", "章节任务"])
        self.queue_table.horizontalHeader().setFixedHeight(28)
        ...
        self.queue_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.Fixed)
        self.queue_table.setColumnWidth(0, 86)
        self.queue_table.setColumnWidth(2, 72)
        ...
        content.addWidget(self.queue_table, 5)
        content.addWidget(self.log_box, 2)
    ...
    def _build_history_panel(self) -> QFrame:
        ...
        self.history_table.setHorizontalHeaderLabels(["漫画", "文件", "路径"])
    ...
    def refresh_history_table(self):
        ...
        for record in self.download_history:
            ...
            values = [record.get("title", ""), record.get("file", ""), record.get("path", "")]
        ...
        if not self.download_history:
            ...
            item = QTableWidgetItem("暂无历史下载记录")
    ...
    def resizeEvent(self, event):
        super().resizeEvent(event)
        if hasattr(self, "result_grid") and self.result_total_count:
            QTimer.singleShot(0, self.refresh_result_layout)
    ...
    def quick_start_download(self):
        ...
        self.add_selected_albums_to_queue()
    ...
    def restore_results_from_cache(self, cache_key: str) -> bool:
        ...
        self.set_result_total(len(cached))
        for index, album in enumerate(cached):
            self.upsert_album_at(index, copy.deepcopy(album))
        self.set_backend_status("已从缓存加载", "ok")
        self.log(f"已从缓存恢复 {len(cached)} 本漫画。")
        return True
    ...
    def start_scrape(self, source_page: int = 1, append: bool = False):
        if self.chapter_worker and self.chapter_worker.isRunning():
            QMessageBox.information(self, "忙碌", "已有漫画加载任务正在运行。")
            return
        ...
        cache_key = self.result_cache_key()
        if not append and self.restore_results_from_cache(cache_key):
        ...
        worker.failed.connect(lambda message, worker=worker: self.task_failed(message) if worker is self.scrape_worker else None)
    ...
    def load_current_chapters(self):
        ...
        self.cancel_btn.setEnabled(True)
        self.switch_page(self.downloading_page_index())
        self.start_loading("正在加载章节")
    ...
    def upsert_album_at(self, index: int, album: AlbumMeta):
        ...
        if not self.results_revealed:
            if self.is_page_loaded(start, end):
                self.reveal_results()
    ...
    def create_album_card(self, album: AlbumMeta) -> QFrame:
        ...
        cover.setStyleSheet("background:#eef3f8;border:1px solid #dde6f0;border-radius:5px;color:#8491a5;")
    ...
    def result_columns(self) -> int:
        if not hasattr(self, "result_scroll"):
            return 4
        return 4 if self.result_scroll.viewport().width() >= 760 else 3
    ...
    def result_card_width(self) -> int:
        ...
        return max(145, min(210, int((viewport_width - margins - spacing) / columns)))

    @staticmethod
    def album_card_style(selected: bool) -> str:
        border = "#1976d2" if selected else "#dfe7f2"
        background = "#eef6ff" if selected else "#ffffff"
        width = "2px" if selected else "1px"
        return (
            f"QFrame#albumCard {{ background:{background}; border:{width} solid {border}; "
            "border-radius:8px; }}"
        )
    ...
    def update_card_selection(self, album_id: str, checked: bool):
        ...
        if checked:
            ...
            self.fade_widget(checkmark, 0.0, 1.0, 160)
        else:
            animation = self.fade_widget(checkmark, 1.0, 0.0, 140)
            animation.finished.connect(lambda: checkmark.setVisible(False))
    ...
    def ensure_detail_dialog(self):
        ...
        self.detail_dialog.resize(380, 560)
        self.detail_dialog.setMinimumSize(340, 460)
    ...
    def change_result_page(self, direction: int):
        ...
        if total_pages <= 0:
            return
        self.current_result_page = max(1, min(total_pages, self.current_result_page + direction))
        start, end = self.current_page_bounds()
        if self.is_page_loaded(start, end):
            self.render_result_page()
            self.result_stack.setCurrentWidget(self.result_scroll)
    ...
    def show_chapters(self, album_id: str):
        ...
        tags = "、".join(album.tags[:30]) if album.tags else "-"
        self.detail_card.setText(
            f"ID: {album.album_id}\n{album.title}\n"
            f"点赞 {album.likes} · 页数 {album.page_count}\n"
            f"作者 {album.author} · 章节 {chapter_count or '-'}\n"
            f"链接：{album.url}"
        )
    ...
    def open_reader_for_current(self):
        ...
        reader = ReaderDialog(album, chapter, self.download_config(), self.log, self)
    ...
    def refresh_queue_table(self):
        ...
        for album in self.queue:
            ...
            self.queue_table.setRowHeight(row, 24)
            chapter_count = len(album.selected_chapter_ids) or len(album.chapters) or 1
            for col, value in enumerate([album.album_id, album.title, str(chapter_count)]):
                item = QTableWidgetItem(value)
                item.setToolTip(value)
                item.setFlags(Qt.ItemIsEnabled | Qt.ItemIsSelectable)
    ...
    def start_download_queue(self):
        if self.is_download_busy():
            QMessageBox.information(self, "忙碌", "已有下载任务正在运行。")
            return
        if not self.queue:
            QMessageBox.information(self, "提示", "下载队列为空。")
            return
        self.download_worker = DownloadWorker(copy.deepcopy(self.queue), self.download_config())
        self.download_worker.log.connect(self.log)
        self.download_worker.progress.connect(self.set_progress)
        self.download_worker.item_done.connect(self.add_completed_item)
        self.download_worker.finished_ok.connect(
            lambda: self.task_done("下载完成。", self.completed_page_index())
        )
        self.download_worker.failed.connect(self.task_failed)
        self.cancel_btn.setEnabled(True)
        self.switch_page(self.downloading_page_index())
        self.start_loading("正在下载漫画资源")
        self.progress.setValue(0)
    ...
    def add_completed_item(self, album: AlbumMeta, pdf_path: str):
        ...
        record = {"title": album.title, "file": path.name, "path": str(path)}
    ...
    def log(self, message: str):
        self.log_box.appendPlainText(time.strftime("[%H:%M:%S] ") + message)
